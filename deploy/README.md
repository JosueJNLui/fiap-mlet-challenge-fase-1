...
Documentação operacional da camada de deploy da API de churn. Esta pasta contém
...
recovery, veja também [`../docs/ARCHITECTURE_DEPLOY.md`](../docs/ARCHITECTURE_DEPLOY.md).
...
|---|---|---|---|---|---|
| Kubernetes | [`helm-chart/`](helm-chart/) + [`helmfile.yaml`](helmfile.yaml) | Cluster Kubernetes local, EKS, GKE, AKS ou similar | Ingress NGINX | ReplicaSet e HPA | stdout coletado pelo cluster |
| AWS ECS | [`terraform/`](terraform/) | Deploy simples e barato em AWS sem operar Kubernetes | Application Load Balancer | `desired_count` do ECS | CloudWatch Log Group |
...
Os defaults ficam em [`helm-chart/values.yaml`](helm-chart/values.yaml), e o
[`helmfile.yaml`](helmfile.yaml) sobrescreve valores para o release `fiap-mlet`.
...
Mais detalhes ficam em [`terraform/README.md`](terraform/README.md).
...
| `MODEL_NAME` | `Churn_MLP_Final_Production` | Nome registrado no MLflow |
| `MODEL_VERSION` | `8` | Pinning determinístico recomendado em produção |
| `PREDICTION_THRESHOLD` | `0.20303030303030303` | Threshold de negócio usado na classificação |
